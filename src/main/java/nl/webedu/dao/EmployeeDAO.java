...
public class EmployeeDAO {
	ConnectDAO connect = new ConnectDAO();
		...
		try {
			insertNewUser = connect.makeConnection().prepareStatement(insertUser_sql);
	...
	public int createEmployee() {
		...
		try {
			...
			insertEmployee = connect.makeConnection().prepareStatement(insertEmployee_sql, Statement.RETURN_GENERATED_KEYS);
	...
	public void unlockEmployee(int emp_id) {
		String lock_query = "UPDATE employee "
				...
				+ "WHERE employee_id = ?";
		try {
			PreparedStatement lock_statement = connect.makeConnection().prepareStatement(lock_query);
	...
	public nl.webedu.models.EmployeeModel loginAssignment(String email, String pw){
		...
		try {
			login_statement = connect.makeConnection().prepareStatement(login_sql);
			login_statement.setString(1, email);
			login_statement.setString(2, pw);
			ResultSet user_set = login_statement.executeQuery();
			
			while(user_set.next()){
				nl.webedu.models.EmployeeModel user = new nl.webedu.models.EmployeeModel(
	...
	public ArrayList<EmployeeModel> getAllEmployees(){
		...
		try {
			PreparedStatement user_statement = connect.makeConnection().prepareStatement(employee_entry_sql);
	...
	public EmployeeModel employee_information(int e_id) {
		...
		try {
			employee_statement = connect.makeConnection().prepareStatement(employee_sql);
	...
	public void editEmployee(EmployeeModel employeeModel){
		...
		String addNewVersionSql = "INSERT INTO employee_version(employee_version_employee_fk," +
				...
				"VALUES(?,?,?,?::enum_role,?,?,?)";
		try {
			PreparedStatement changeVersions= connect.makeConnection().prepareStatement(oldVersionDisableSql);
			changeVersions.setInt(1, employeeModel.getEmployeeId());
			changeVersions.executeUpdate();
			changeVersions.close();


			PreparedStatement addVersionStatement = connect.makeConnection().prepareStatement(addNewVersionSql);
